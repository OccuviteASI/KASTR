...
- **The relay asks, KASTR answers.** moq-relay 0.15 verifies nothing itself: one JSON event per session (`connect`,
  `revalidate`, `end`; measured keys `alpn, event, id, node, path, query (only with ?jwt=), remote, server_name,
  transport`, no role) goes to `[auth] url`, and the answer is a grant of publish/subscribe patterns. KASTR's token
  service became that server; it verifies the same 0.13-shaped tokens with the same key, so nothing a client holds
  changed. Reply first, log after -- the relay is waiting.
- **Pin the certificate, not the hostname.** The hub's QUIC certificate is regenerated on every hub start, so a pin is a
  value that moves. Learn it from the hub's own token service, store it beside the cluster config, and re-learn on the
  relay's own complaint (`InvalidContentType`, `cluster peer error`) -- debounced, and restarting only when the pin
  actually changed, so a hub that is merely down never churns its spokes. Rig: re-pinned and rejoined within 30 s.
- **A shared connection is shared.** `@moq/net` 0.4 removed `Reload` and `established`; connections are shared per URL
  and refuse per-instance options (`delay`). Everything publishes and consumes through `connection.origin`; announce
  discovery is a stream of route entries (`announced` / `retracted` / `updated`) that survives a relay bounce by
  emitting retractions and re-announces (rig: 12 s and 150 s outages both recovered without a reload).
- **A pointer into the DOM must survive a rebuild.** The own-camera spotlight died because the pane node is replaced by
  every self-heal (no-echo rebuild, viewer stall, encoder stall, hardware fallback) and the spotlight held the old node.
  Give the pane a stable id and re-resolve by it; never trust `isConnected` alone.
- **Two encoders on one CPU: the nicer one loses.** The media transcode ran at normal priority on every core beside the
  browser's own encoder. Run the one nobody is watching below normal on half the cores, draw at the file's own rate,
  and show the owner the encoder's real output so the field can read it in one line.
- **Loop is an opt-in and pause is a fact.** Five paths could restart a paused share; one flag (`userPaused`) now gates
